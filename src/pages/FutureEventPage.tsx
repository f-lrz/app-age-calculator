import { useState, useCallback } from 'react';
import { useForm, type SubmitHandler } from 'react-hook-form';
import DateForm, { type IFormInput } from '../components/DateForm';

interface TimeRemaining {
  years: number | null;
  months: number | null;
  days: number | null;
}

const FutureEventPage = () => {
  const [time, setTime] = useState<TimeRemaining>({ years: null, months: null, days: null });
  
  // O hook useForm agora vive aqui e controla todo o estado do formulário.
  const {
    register,
    handleSubmit,
    formState: { errors },
    setError,
    clearErrors,
  } = useForm<IFormInput>();

  const calculateTimeRemaining = useCallback((futureDate: Date): TimeRemaining => {
    const today = new Date();
    let years = futureDate.getFullYear() - today.getFullYear();
    let months = futureDate.getMonth() - today.getMonth();
    let days = futureDate.getDate() - today.getDate();

    if (days < 0) {
      months--;
      days += new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
    }
    if (months < 0) {
      years--;
      months += 12;
    }
    return { years, months, days };
  }, []);

  // Esta função agora contém TODA a lógica de validação.
  const handleCalculateEvent: SubmitHandler<IFormInput> = (data) => {
    clearErrors();
    const { day, month, year } = data;
    const eventDate = new Date(year, month - 1, day);
    const today = new Date();
    let isValid = true;
    
    today.setHours(0, 0, 0, 0);

    // Validação 1: Data inválida (ex: 31/06/2045)
    if (eventDate.getFullYear() !== year || eventDate.getMonth() !== month - 1 || eventDate.getDate() !== day) {
        setError('day', { type: 'manual', message: 'Invalid date' });
        setError('month', { type: 'manual', message: '' });
        setError('year', { type: 'manual', message: '' });
        isValid = false;
    }

    // Validação 2: Data no passado
    if (eventDate < today) {
      setError('year', { type: 'manual', message: 'It must be in the future' });
      isValid = false;
    }

    if (!isValid) {
      setTime({ years: null, months: null, days: null });
      return;
    }

    const timeRemaining = calculateTimeRemaining(eventDate);
    setTime(timeRemaining);
  };

  return (
    <div className="bg-white rounded-3xl rounded-br-[100px] md:rounded-br-[150px] p-8 shadow-lg max-w-2xl w-full">
      <h2 className="text-xl font-bold mb-4 text-gray-700">Future Event Calculator</h2>
      {/* Passamos todas as funções e estados do formulário para o componente DateForm */}
      <DateForm
        handleSubmit={handleSubmit}
        onFormSubmit={handleCalculateEvent}
        register={register}
        errors={errors}
      />

      <div className="text-5xl md:text-8xl font-extrabold italic">
        <h1>
          <span className="text-purple-600">{time.years ?? '--'}</span> years
        </h1>
        <h1>
          <span className="text-purple-600">{time.months ?? '--'}</span> months
        </h1>
        <h1>
          <span className="text-purple-600">{time.days ?? '--'}</span> days
        </h1>
      </div>
    </div>
  );
};

export default FutureEventPage;

